...
# begin wxGlade: EB::Wx::Report::BTWAangifte::Preferences::__do_layout
	...
	$self->{sz_buttons}->Add($self->{b_cancel}, 0, wxLEFT|wxRIGHT|wxBOTTOM|wxEXPAND|wxADJUST_MINSIZE, 5);
	$self->{sz_buttons}->Add($self->{b_ok}, 0, wxLEFT|wxRIGHT|wxBOTTOM|wxEXPAND|wxADJUST_MINSIZE, 5);
	$self->{sz_prefs}->Add($self->{sz_buttons}, 0, wxEXPAND, 5);
