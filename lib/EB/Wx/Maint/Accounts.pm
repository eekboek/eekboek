...
# begin wxGlade: AccPanel::new
	...
	$self->{b_reset} = Wx::Button->new($self->{maint_pane}, -1, "Herstellen");
...
# end wxGlade
	...
	my $closehandler = sub {
	    ...
	    $config->accw->{xpos} =  $a[0];
	    $config->accw->{ypos} =  $a[1];
	    @a = $self->GetSizeWH;
	    $config->accw->{xwidth} = $a[0];
	    $config->accw->{ywidth} = $a[1];
	};
	...
	EVT_SPLITTER_SASH_POS_CHANGED($self->{w_acc_frame},
				      ...
				      sub {
					  my ($self, $event) = @_;
					  $config->set("accsash", $self->GetSashPosition);
				      });
...
# begin wxGlade: AccPanel::__do_layout
	...
	$self->{w_acc_frame}->SplitVertically($self->{tree_pane}, $self->{maint_pane}, );
